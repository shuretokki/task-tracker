...
std::string getDataPath()
{
    ...
    if ( ! dataPath.empty() ) {
        fs::path appFolder { dataPath / "ctask" };
    ...
    for ( size_t i = 1; i < argc; ++i ) {
        ...
        if ( ARGUMENT == "-V" || ARGUMENT == "--VERSION" ) {
            ...
            try {
                ...
                if ( ! is_task_found ) {
                    std::cerr << "[!] TASK WITH ID " << targetId
                              << " NOT FOUND\n";
                    return 1;
                }
          ...
          ARGUMENT == "MARK-IN-PROGRESS" || ARGUMENT == "IN-PROGRESS" ) {
            READ JSON_ ( DATA_PATH );
            if ( ! JSON_.is_open() ) {
            ...
            if ( argc == 2 ) {
                ...
                if ( FILTER == "-L" || FILTER == "--LONG" ) {
                  ...
                  FILTER == "TODO" || FILTER == "TO-DO" || FILTER == "-T" ) {
                    for ( auto& task : j ) {
                        if ( ! (
                               task.is_object()
                               && task [ "status" ] == "TO-DO" ) ) {
